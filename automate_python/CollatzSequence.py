# https://automatetheboringstuff.com/2e/chapter3/ Practice Project
...
# to an integer with the int() function; otherwise, it will be a string value.
...
# 1
# Input Validation
# Add try and except statements to the previous project to detect whether the user types
#  in a noninteger string. Normally, the int() function will raise a ValueError error
#  if it is passed a noninteger string, as in int('puppy'). In the except clause,
...
try:
    ...
    while True:  # the main program loop
        if result != 1:
            output = collatz(result)
            if result != 1:
                result = collatz(output)
